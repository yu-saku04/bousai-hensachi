...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const url = `${SITE_URL}/result/${encodeURIComponent(data.prefecture)}/${encodeURIComponent(data.municipality)}`;
...
export default async function ResultPage({ params }: PageProps) {
  ...
  const SITE_URL = "https://bousai-hensachi.vercel.app";
  const pageUrl = `${SITE_URL}${buildResultPath(data.prefecture, data.municipality)}`;
    ...
    <div className="min-h-screen bg-gradient-to-b from-blue-50 to-white">
      ...
      <div className="mx-auto max-w-md px-4 py-6 space-y-5">
        ...
        <section className="bg-white rounded-2xl border border-gray-100 p-5 shadow-sm">
          ...
          <RadarChartWrapper municipality={data} />
        </section>
